...
const CONFIG = {
  ...
  particleCount: 160000,    // AURORA ver11: was 45000 in ver10. The customer read the sparse
                            ...
                            //   Cost is linear in this and nothing else here is, so it is
                            //   the number to check on real hardware before shipping.
  ...
  attractCore: 0.30,        // as a fraction of the reach. INSIDE this the pull eases off,
                            //   which is what makes it gather rather than collapse:
...
vec3 attractTo(vec3 p, vec3 target){
  ...
  // The smoothstep alone was zero AT the target — a dead zone the size of the core where
  // motes felt no pull and simply rode the field's current, which near the tabs drifts
...
// AURORA ver10: the flowing transition. The pull point is sprung behind the CSS box,
// lifted into an arc while in motion, and the curl is strengthened while travelling —
// so the population streams along a curved, lagging path and settles softly on the
// tab, instead of translating linearly with the box.
...
function updateAttract(vh, dt) {
  ...
  if (el) {
    ...
    // the arc: lift the target while in motion, ease the lift away at rest, so the
    // path between tabs is a shallow curve over the row rather than a straight line
    const arcTarget = Math.sin(Math.min(Math.PI, speed * 2.6)) * 0.085;
...
function tick() {
  ...
  // before the sim step, so the cloud flows after it. dt drives the sprung pull point.
  updateAttract(viewHeightAt(CONFIG.anchorZ), dt);
