...
void moundState(float across, float shape, float time,
  ...
  // A SLIM peak — narrower than a tab — so the silhouette is an elegant needle of light
  // over the selected category, not a heap.
  ridge = exp(-pow(across - uPeakX, 2.0)
              * (64.0 + 16.0 * sin(flowTime * 0.30 + shape * 6.2831)));
...
void main(){
  ...
  pos.y = height * uMound.y * (0.02 + 0.98 * ridge) * edgeDrop
  ...
  // between the sink line and the limit the grains thin out as they go down. VER4's
  // minimalism: a grain's presence follows the ridge — away from the peak the line is
  // only a whisper, so the form is one slim peak of light on a hairline.
  vFade = envelope * (1.0 - smoothstep(0.72, 1.0, aShape)) * cropEnv
        * (0.22 + 0.78 * pow(ridge, 0.55));
...
function place() {
  ...
  uniforms.uMound.value.set(
    ...
    ((floor - y * innerHeight + 38) / ppw / group.scale.x) * 1.12,  // VER4: the slim peak rises a little taller
