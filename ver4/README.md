# PTSBottomVer4 — "minimal ridge"

Variant of `PTSBottomVer1` answering the note that the accumulation read as a heavy sand heap. Same engine, same DNA, reduced to a hairline: the floor band is nearly flat and its grains fade with the ridge, so away from the peak the layer is only a whisper, and the form is one slim, slightly taller peak of light over the selected tab.
