...
const CONFIG = {
  ...
  particleSize: 1.70,        // VER8: finer grains — the halo reads as dust, not as paint
  ...
  // ------------------------------------------------------------ arrival
  // The cloud is born at its seats and unfurls from them, and the first second of that is a
  // tight knot rather than a mass — it is the simulation reaching its steady state in front
  // of the viewer. These run it BEFORE the first frame is shown, so what arrives is a cloud
  // that is already the shape it settles at.
  //
  ...
  warmSeconds: 2.0,         // seconds of simulation to run before anything is drawn
  ...
  warmCeilingMs: 2500,      // hard stop on the whole warm-up. A slow device gets a cloud that
                            //   is only part-warmed, which is far better than a spinner: this
                            //   is a decoration, and it must never be the reason a page waits
  fadeInSeconds: 0.6,       // the ink comes up over this once the warm-up is done, so the
  ...
  // ver14 ships at 1, so the whole hover is the force below. A displacement is a pure
  // function of where the pointer is NOW: its disturbance travels rigidly with the cursor,
  ...
  hoverHold: 0.50,          // seconds for a shove's own velocity to fall to a third of
  ...
  // (1 - settle)): about 59 at the heavy end and 1 at the light one. So the same number means
  ...
  // occupies. Stamps are dropped as the cursor travels and each fades on its own clock, so a
  ...
  hoverTrailSpacing: 0.40,  // how far the pointer must travel before a new stamp is laid, as
                            ...
                            //   would multiply the push by however long it was held still

  // ---- the shape ------------------------------------------------------------------
  // A DOMAIN WARP, and it is the difference between a ragged circle and a shape that is not
  // a circle at all. Perturbing the RADIUS only moves the boundary - the field inside it is
  // still a disc measured from one point, so every contour under the rim stays round and the
  ...
  mouseWarpScale: 2.2,      // warp features across one reach. Around two gives a few lobes
                            ...
                            //   shape
  // How much of the push is TANGENTIAL rather than straight out from the pointer.
  // A purely radial push is obliged to make a rim: it sweeps motes outward, they slow where
  ...
  // ------------------------------------------------------------ the label's pull
  // The sign is the fixed thing on the page and the cloud belongs to it. The seats are
  // already crowded under it — focusX/focusY put the density peak on the label's own box —
  // but that is where the motes are BORN, and after that the field carries them wherever it
  // goes. This is the standing force that keeps bringing them back, so the mass gathers on
  // the sign instead of merely starting there.
  //
  ...
  attractPull: 6.0,         // VER8: the gather's lift to the tab's height is ~11 sim units of
                            //   climb, and the pull is a fraction of the sim radius per
                            //   second — this crosses it in a couple of seconds. (VER3's
                            //   floor-hugging cloud only ever crossed ~0.2 sim, which is
                            //   why its 0.50 gathered the sides but never lifted.)
  ...
  attractCore: 0.50,        // VER8: a WIDE core — the halo is the CTA's airy plume, not a
                            // packed bead. INSIDE it the pull eases off to
  ...
  // simulation texture, same draw call, same shaders. A mote belongs to the text's group if
  // its SEAT falls in the label's box, and that is decided once when the seats are thrown.
  //
  // The text's group is born on the label, stays there, and ignores the pointer. The rest
  // drift, bloom and answer the cursor exactly as they did before this existed. The total
  // count does not change, so nothing is added for a phone to render — the only cost is a
  // handful of arithmetic per mote per frame, against a curl field and a lit sphere.
  //
  ...
  alphaGain: 0.30,          // VER8: fainter than the band's 0.43 — the gathered halo holds
                            //   the whole population in a small area, so the per-grain alpha
...
// ?push reaches hoverPush, which is the strength the FORCE is stated in. mouseStrength
// belongs to the displacement, and the displacement is off unless ?hover is turned down.
if (numParam('push', 0, 2) !== null) CONFIG.hoverPush = numParam('push', 0, 2);
...
if (numParam('seats', 0, 0.9) !== null) CONFIG.signSeats = numParam('seats', 0, 0.9);
...
void moundState(float across, float shape, float time,
  ...
  // VER3: the mound itself is capped — the surface is only a low dusting along the floor,
  // and the FORM comes from the gathered cloud at the tab, not from the heap.
  ridge = 0.15 * exp(-pow(across - uPeakX, 2.0)
              * (26.0 + 6.0 * sin(flowTime * 0.30 + shape * 6.2831)));
  ...
  calm = 1.0 - 0.85 * exp(-pow((across - uPeakX) * 2.8, 2.0));
...
// How many stamps of the pointer's path the shader carries. It is the reach of the memory
// in SPACE, the way hoverTrail is its reach in time: at hoverTrailSpacing of the reach
// between stamps, this many of them cover STAMP_SLOTS * spacing * reach of travel, and once
// the array is full the OLDEST is dropped. At 8 that was 3.2 reaches — a quarter of the
// frame — so a push given on the right had been thrown away by the time the pointer reached
// the left and the two could never be in the cloud together.
...
uniform float uMouseWarpScale;
uniform float uSwirl;
// The trail: xyz is a stamp's ray origin in this object's space and w its weight, with the
...
// The label's pull. A standing force toward the sign, shaped so that it gathers rather than
// collapses: it eases IN from nothing at the centre, so a mote that has arrived is no longer
// pulled and the ones behind it are not packed on top of it, and it eases OUT to nothing at
// the reach, so this is a local gathering and not a well the whole corner falls into.
//
...
// pointer took rather than gathered at the point it happens to be on.
//
// What it returns is an acceleration the velocity then CARRIES, which is the whole
// difference: a displacement exists only where the pointer is and vanishes the instant it
// leaves, where a force leaves momentum behind. Push from the left and then from the right
// and both are still in the cloud at once, because the cloud is holding them in its
// velocities and not in the pointer's position.
//
// Three things stop it reading as a torch shone at one place, and they answer three
// different halves of the same complaint:
...
vec3 cursorForce(vec3 here, float shape){
  ...
  // direction, bent off radial, so the push is not a star out of one point
  float bend = snoise3dDeriv(np * 1.3 + vec3(47.0, 3.0, 29.0)).w * uMouseNoise * 1.6;
  float cb = cos(bend), sb = sin(bend);
  // and this mote's own reach, which turns the boundary into a band as wide as the spread
  float mr = uMouseRadius * max(0.15, 1.0 + (shape - 0.5) * 2.0 * uMouseEdgeBlur);
  ...
  for (int i = 0; i < ${STAMP_SLOTS}; i++) {
    ...
    if (w <= 0.002) continue;
    vec3 dir = uStampD[i];
    vec3 toP = wp - uStampO[i].xyz;
    // distance to the pointer RAY, not to a point on a plane, so the cloud answers through
    // its whole depth instead of at one z
    vec3 delta = toP - dir * dot(toP, dir);
    float d = length(delta);

    // No cut-off, and so no boundary. A bounded falloff ends somewhere, and wherever it ends
    // the motes just inside it have been moved and the ones just outside have not, which
    // draws the edge back on however the curve is shaped. This one has a tail instead: down
    // to a twentieth at the nominal reach and immeasurable at twice it.
    float f = exp(-3.0 * pow(d / mr, uFalloffPower)) * w * gain;
    if (f < 0.0005) continue;

    vec3 radial = delta / max(d, 1e-4);
    // the swirl turns about the pointer's own ray, so it lies in the plane the cloud is seen
    // in whatever the camera is doing
    vec3 tangent = cross(dir, radial);
...
void main(){
  ...
  // The field is damped in a zone about the gather point, so the gathered mass over the
  // tab sits still and centred instead of being pushed about — or pushed sideways by the
  // flow's own lean — by the churn. Outside the zone the field runs at full strength.
  ...
  // jitter (uAttractSpread), so the population fills a VOLUME around the tab instead of
  // packing into one bead — the airiness the CTA's plume has. Each grain answers on its
  // own clock (the seed-scaled factor) and the field keeps churning underneath, so the
  // cloud eddies instead of packing solid.
...
void main(){
  ...
  worldSize *= grow;
  // and it carries more ink than the free cloud, because it is the one part of the picture
  // that has to be read AGAINST rather than merely seen
...
// state this runs the same spring off the same input. The constants below are the wall's and
// have to be kept in step with them.
//
// Taking the same magnification is exact rather than approximate, and it is worth knowing
// why: camera.zoom is a pure magnification of the projected image about the screen centre, so
// both scenes move every point radially out from that centre by the same factor. A mote stays
// over the point of the wall it started on whatever depth either scene puts its geometry at,
// with no shared camera and nothing to keep in sync.
const ZOOM = {
  max: 1.6,        // scroll-in target
  screens: 6,      // scroll distance for the full range, in viewport heights
  spring: { stiffness: 80, damping: 22, mass: 0.7 },
};

// How much of the wall's magnification the cloud takes. 1 is the wall's own, which pins it to
// the plaster; 0 leaves it fixed on the screen at its own size.
//
// ver17 ships 0, which is ver14's behaviour and what the client asked to go back to. Pinning
// is exact — a mote holds its point of the wall — but the cloud's centre sits ON the screen
// corner, so a magnification about the screen centre carries the whole mass out of frame and
// a full scroll costs the effect: 13,534 drawn ink pixels at rest against under 50 at full
// zoom. `?pin=1` is ver15/ver16's default back, and `?hold=1` below is the middle answer.
const ZOOM_PIN = numParam('pin', 0, 1) !== null ? numParam('pin', 0, 1) : 0;

// The cloud's centre sits ON the screen corner, so magnifying about the screen centre carries
// it outward and off the frame — correct, since the piece of wall it is pinned to leaves the
// frame too, but it means a scroll costs the effect. ?hold=1 keeps the cloud's own anchor
// where it is on screen while everything inside it still magnifies with the wall, so the mass
// grows in place instead of travelling. It is no longer pinned to one point of the plaster,
// which is the trade.
const ZOOM_HOLD = PARAMS.get('hold') === '1';

// the magnification the cloud is currently taking; place() reads it
let zoomFactor = 1;
...
function buildParticles(count) {
  ...
  // ---- the seats that belong to the text ----------------------------------
  // Thrown last, over the ones the model gave, so this group exists whatever the silhouette
  // did or did not put under the words. The box is the element's own, converted into the
  ...
  if (signCount > 0) {
    ...
    if (el) {
      ...
      // Even across the BOX — the words need their ends backed as much as their centre, and
      // a rectangle is what text is.
...
// The pointer's trail, as the shader sees it. Allocated once and written in place.
const stampO = [];
const stampD = [];
...
function makeSim() {
  ...
  const step = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uMouseNoise: { value: CONFIG.mouseNoise },
      uMouseNoiseScale: { value: CONFIG.mouseNoiseScale },
      uMouseWarp: { value: CONFIG.mouseWarp },
      uMouseWarpScale: { value: CONFIG.mouseWarpScale },
      uSwirl: { value: CONFIG.hoverSwirl },
      // the same array objects updateCursor writes into every frame: three holds on to the
      ...
      // so where the sign is in the cloud's own space — is known.
      uSignHalf: { value: new THREE.Vector2(1e-5, 1e-5) },
...
function stepSim(dt) {
  ...
  // No hoverFade here any more, and that is deliberate. The fade was the arrive/leave ease
  // for a displacement, which needs one because it appears at full size in a single frame.
  // A force does not: the velocity integrating it IS the ease in, and on the way out the
...
function place() {
  ...
  if (ZOOM_HOLD && zoomFactor !== 1) {
    group.position.x /= zoomFactor;
    group.position.y /= zoomFactor;
  }
  ...
  if (sim) {
    // VER8: the pull's shared target IS the selected tab — every grain gathers there —
    // with the cloud's centre lifted to the tab's own height (a touch above the text) so
    // the CTA halo reads ON the menu, not under it. The lift is stated in screen pixels
    // and converted into group-local units (÷ ppw·scale), then into sim y through the
    // vertical mapping the vertex pass applies (0.40 × the calm-blend; at the tab itself
    // calm is 0.15). The world is ~vh units per frame, NOT pixels — every conversion has
    // to go through ppw.
    const yFactor = 0.40 * (0.35 + 0.65 * 0.15);
    const pxToLocal = 1 / (ppw * group.scale.x);
    const liftLocal = ((floor - y * innerHeight) + 0.02 * innerHeight) * pxToLocal;
    sim.step.uniforms.uMigrateX.value = peakAcross * uniforms.uMound.value.z;
    sim.step.uniforms.uAttractPoint.value.set(
      peakAcross * uniforms.uMound.value.z, liftLocal / yFactor, 0.0);
    uniforms.uSignPoint.value.set(0, 0, 0);
    // The grip is measured in world units now, so the reach is stated as a fraction of
    // the frame: wide enough that the whole population feels the gather from anywhere on
    // the menu, with the core keeping the halo about one tab-and-a-half wide.
    sim.step.uniforms.uAttractRadius.value = (vh / group.scale.x) * 0.40;
    sim.step.uniforms.uAttractWorldScale.value.set(
      uniforms.uMound.value.x / uniforms.uMound.value.z, yFactor, 1.0);
    // The halo's VOLUME: each grain's aim is jittered about the shared point, so the
    // plume breathes around the tab — wider than tall, like the CTA's flare — instead of
    // packing into a bead. Stated in sim units through the same mappings as the grip.
    sim.step.uniforms.uAttractSpread.value.set(
      0.55 * uniforms.uMound.value.z,            // ±0.27 across → ±~200px on a 1440 frame
      (vh / group.scale.x) * 0.20 / yFactor,
      2.0);
    simLeash = (vh / group.scale.x) * 1.8;
  }
...
// the same place: the shader sums the stamps, so stacking would quietly multiply the push by
// how long the pointer was held still.
const trail = [];
let trailClock = 0;

function updateTrail(o, d, dt) {
  trailClock += dt;
  const reach = uniforms.uMouseRadius.value;

  if (seenPointer && pointerInside && reach > 0) {
    const last = trail.length ? trail[trail.length - 1] : null;
    if (last && last.o.distanceTo(o) < CONFIG.hoverTrailSpacing * reach) {
  ...
  const life = Math.max(0.001, CONFIG.hoverTrail);
  while (trail.length && trailClock - trail[0].t > life * 5.6) trail.shift();

  for (let i = 0; i < stampO.length; i++) {
    const st = i < trail.length ? trail[i] : null;
    if (!st) { stampO[i].set(0, 0, 0, 0); continue; }
    const w = Math.exp(-(trailClock - st.t) / life);
    stampO[i].set(st.o.x, st.o.y, st.o.z, w < 0.004 ? 0 : w);
    stampD[i].copy(st.d);
  }
}
...
function updateCursor(dt) {
  ...
  updateTrail(localOrigin, localDir, dt);
...
sim = makeSim();
// once more now the simulation exists: place() writes the label's pull into its uniforms, and
// the first call came from resize() before there was anything to write into
place();
...
function updateZoom(dt) {
  ...
  const wall = Math.max(1, 1 + (ZOOM.max - 1) * zoomSpring.x);
  const z = 1 + (wall - 1) * ZOOM_PIN;
  if (z === zoomFactor) return;
  zoomFactor = z;
  camera.zoom = z;
  camera.updateProjectionMatrix();
  // the anchor, the cursor's reach and the bloom's origin are all derived from the frame, so
  // they are re-derived here rather than left at the values the last magnification implied
  place();
}

// Simulated seconds already run, wall clock spent running them, and the ink's way in.
let warmed = 0;
let warmSpentMs = 0;
let fadeIn = 0;
let fadeSettled = false;

// Advance the simulation without drawing. Nothing else is touched: no cursor, no parallax,
// no placement — the pointer has not been anywhere yet and the group's transform does not
// depend on the clock.
function warmUp() {
  ...
  do {
    ...
    warmed += step;
  } while (warmed < CONFIG.warmSeconds && performance.now() - t0 < CONFIG.warmBudgetMs);
  warmSpentMs += performance.now() - t0;
}
...
function tick() {
  ...
  sortByDepth();
  updateCursor(dt);
  // The ink comes up on its own curve rather than the canvas's opacity, because the canvas
  // carries the wall as well and fading that would fade the surface the cloud stands on.
  // Squared, so it starts from nothing rather than from a visible step. Once it is home the
  // uniform is set exactly and never written again, which leaves the density bar owning it.
  if (!fadeSettled) {
    fadeIn = CONFIG.fadeInSeconds > 0 ? Math.min(1, fadeIn + dt / CONFIG.fadeInSeconds) : 1;
    uniforms.uAlphaGain.value = CONFIG.alphaGain * fadeIn * fadeIn;
    if (fadeIn >= 1) { uniforms.uAlphaGain.value = CONFIG.alphaGain; fadeSettled = true; }
  }
