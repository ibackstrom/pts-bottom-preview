...
const CONFIG = {
  ...
  flowNoise: 1.2,           // the NOISE dial: the organic surface noise, the same
...
void moundState(float across, float shape, float time,
  ...
  // VER5 «reaching»: a NARROW, TALL column — the band's grains stretch up from the floor
  // toward the chosen tab like wisps of smoke reaching for it, instead of heaping into
  // a mound. The coefficient is nearly triple the mound's so the wisp is slim, and a slow
  // breathing term keeps the column's width alive so it never reads as a solid pillar.
  ridge = exp(-pow(across - uPeakX, 2.0)
              * (110.0 + 22.0 * sin(flowTime * 0.30 + shape * 6.2831)));
...
void main(){
  ...
  // category, so the population keeps its spread while it rolls — grain by grain, through
  // the field, not as one sheet being carried across. Each grain answers on its own clock
  // (the seed-scaled factor), which keeps the migration front ragged, and the field keeps
  // churning underneath so the paths wander instead of running parallel.
  v += attractForce(here, seed.xyz + vec3(uMigrateX, 0.0, 0.0))
...
void main(){
  ...
  // VER5: the floor is a hairline — the form lives almost entirely in the ridge, so the
  // reaching column carries the silhouette rather than a heap under it.
  pos.y = height * uMound.y * (0.03 + 0.97 * ridge) * edgeDrop
  ...
  // between the sink line and the limit the grains thin out as they go down. VER5's
  // lightness: a grain's presence follows the ridge — off the reaching column the band
  // is only a whisper, so the form is wisps stretching up, not sand.
  vFade = envelope * (1.0 - smoothstep(0.72, 1.0, aShape)) * cropEnv
        * (0.10 + 0.90 * pow(ridge, 0.45));
...
function place() {
  ...
  uniforms.uMound.value.set(
    ...
    // VER5 «reaching»: the column stretches 3.2x taller than the mound did — from the
    // floor at the menu up toward the page's middle, reaching for the chosen category.
    ((floor - y * innerHeight + 38) / ppw / group.scale.x) * 3.2,
