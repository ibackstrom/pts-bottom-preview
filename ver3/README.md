# PTSBottomVer3 — "CTA cloud"

Variant of `PTSBottomVer1` answering the note that the accumulation read as a heavy sand heap. Same engine, same DNA, but the form is the CTA's: the mound is capped to a low dusting along the floor, and the pull gathers the whole population into one soft breathing cloud that hangs at the selected tab — the same gather-and-eddy behaviour as the label cloud in the original PTS system.
