...
const CONFIG = {
  ...
  attractPull: 0.50,        // VER3: strong enough to gather the WHOLE population into the
  ...
  attractCore: 0.35,        // VER3: a WIDE core, so the gather is a loose breathing halo
...
void main(){
  ...
  // The gather. VER3 pulls every grain toward the SHARED point — the selected tab,
  // lifted a little off the floor — so the population gathers into one soft, breathing
  // cloud that hangs at the category, the way the label cloud does in the CTA. Each
  // grain answers on its own clock (the seed-scaled factor) and the field keeps
  // churning underneath, so the cloud eddies instead of packing solid.
  v += attractForce(here, uAttractPoint)
...
function place() {
  ...
  if (sim) {
    ...
    // VER3: the pull's shared target IS the selected tab — every grain gathers there —
    // lifted well off the floor so the cloud hangs rather than rolling along the floor
    // line, and the reach is tightened so the gather is a round halo, not a wave.
    sim.step.uniforms.uAttractPoint.value.set(
      peakAcross * uniforms.uMound.value.z, uniforms.uMound.value.z * 1.1, 0.0);
    uniforms.uSignPoint.value.set(0, 0, 0);
    sim.step.uniforms.uAttractRadius.value = uniforms.uMound.value.z * 0.9;
