...
const CONFIG = {
  ...
  particleSize: 1.7,         // sphere diameter, world units, before the per-mote multiplier
...
void moundState(float across, float shape, float time,
  ...
  // A NARROW spine — a fraction of a tab wide — so the mound reads as a thin luminous
  // filament rising from the line, not as a heap of sand.
  ridge = exp(-pow(across - uPeakX, 2.0)
              * (90.0 + 20.0 * sin(flowTime * 0.30 + shape * 6.2831)));
...
void main(){
  ...
  pos.y = height * uMound.y * (0.025 + 0.975 * ridge) * edgeDrop
  ...
  // between the sink line and the limit the grains thin out as they go down. VER2's
  // lightness lives in the FORM — the thin line and narrow peak — not in dimming: the
  // layer keeps most of its presence so the hover's wake still reads.
  vFade = envelope * (1.0 - smoothstep(0.72, 1.0, aShape)) * cropEnv * 0.82;
